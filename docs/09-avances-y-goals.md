...
- Hardening remoto inicial 2026-05-22: Supabase Auth remoto quedó con Site URL/Redirect URLs de Vercel, confirmación de email, contraseña mínima de 12 caracteres, requisito de minúscula/mayúscula/número, reautenticación para cambio de clave y SSL enforcement externo; la app quedó preparada para Cloudflare Turnstile mediante `NEXT_PUBLIC_TURNSTILE_SITE_KEY`.
...
- Mantener Auth/RLS como base obligatoria; antes de usuarios reales configurar en Supabase remoto SMTP real, Google OAuth si aplica, CAPTCHA con claves reales, verificación de Realtime `Allow public access` y pruebas multiusuario.
...
| Auth, RLS y ownership | Resuelta base productiva | Supabase Auth email/password, OAuth Google opcional, callback `/auth/callback`, onboarding mínimo, roles y policies RLS implementados y testeados | Endurecer SMTP/confirmación de email y configurar Google OAuth en staging/producción; administrar miembros desde UI en goal posterior | `supabase/migrations/`, `supabase/tests/rls.sql`, `app/login`, `app/auth/callback`, `app/onboarding`, `docs/08-produccion.md` |
